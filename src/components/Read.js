...
export const Read = () => {       
  ...
  const getData=async()=>{ 
    try 
    { 
       const response=await axios.get("https://bend-crud-node-express-kb0t.onrender.com/api/v1/employee"); 
  ...
  const handleDelete=async(id)=>{ 
    try 
    { 
       await axios.delete(`https://bend-crud-node-express-kb0t.onrender.com/api/v1/employee/${id}`)
       getData();
    }catch(error) 
    { 
